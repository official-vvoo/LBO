...
// Functoin for Json; made by Bemoit Blanchon v7.0.4
...
// KYR: Function for now Time(Tick) 
...
// KYR: Function for String -> Json for Transmitt
...
// KYR: function for address, Mac address must be equal value at addr of RPI code
...
// KYR: Function for Json -> String & Print data at Serial Monitor
  ...
  for(;;){
    if (Rx_front != Rx_rear) {
      ...
      if(xSemaphoreTake(rx_semaphore_b, portMAX_DELAY) == pdTRUE) {
        ...
        rx_servo = (int) Rx_doc["data"]["servo"];
        rx_beep = (int) Rx_doc["data"]["beep"];
...
// KYR: Function for Send Data with bluetooth
void Task_Send_Tx_Data(void *params) {
  for(;;) {
    ...
    // Serial.print("Time: " + String(_time) + " [gyro_x: " + String(gyro_x) + "] [gyro_y: " + String(gyro_y) + "] [gyro_z: " + String(gyro_z) + "] [gyro_w: " + String(gyro_w) + "] ");
...
// KYR: Function for collect Receive data with Interrupt. 1Byte by 1Byte
...
// KYR: Function for bluetooth Init and Create bluetooth tasks
